...
let publicEvents=[],publicVideos=[],publicRecruitments=[],publicDataLoading=true;
...
function renderCalendar(){
 ...
 if(!items.length){if(publicDataLoading&&!$('demo').checked)$('event-list').append(empty('ライブ情報を読み込んでいます…','そのまま少しお待ちください。'));else $('event-list').append(empty($('demo').checked?'条件に合うライブはありません':'まだライブが掲載されていません。',$('demo').checked?'別の日付や地域を選んでお試しください。':'別の日付や地域もご確認ください。'));}
...
renderCalendar();renderTags();renderRecruitments();
...
publicDataLoading=false;$('poster-carousel').removeAttribute('aria-busy');

...
function showSaveToast(message,state='success'){
  const toast=$('save-toast');clearTimeout(saveToastTimer);toast.textContent=message;toast.dataset.state=state;toast.hidden=false;
  saveToastTimer=setTimeout(()=>{toast.hidden=true;},5000);
}
function finishSuccessfulSave(panel,message){
  $('review-dialog').close();selectPanel(panel);
  ...
  try{
    ...
    for(const [title,type,items] of groups){
      ...
      for(const item of items){
        ...
        actions.append(button);row.append(text,actions);section.append(row);
      }
      list.append(section);
    }

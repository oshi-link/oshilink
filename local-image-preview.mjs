...
const outputSize=kind=>kind==='avatar'?{width:800,height:800}:{width:1500,height:500};
function drawCrop(canvas,editor,kind){
  const output=outputSize(kind),source=cropArea(editor.source.naturalWidth,editor.source.naturalHeight,output.width,output.height,editor.zoom,editor.position.x,editor.position.y);
  if(canvas.width!==output.width)canvas.width=output.width;if(canvas.height!==output.height)canvas.height=output.height;
  const context=canvas.getContext('2d');context.clearRect(0,0,output.width,output.height);context.drawImage(editor.source,source.x,source.y,source.width,source.height,0,0,output.width,output.height);
}
async function renderCrop(editor,kind){
  const canvas=document.createElement('canvas');drawCrop(canvas,editor,kind);
...
export function bindImagePreview({input,image,frame,notice,remove,placeholder,kind,title}){
  ...
  const stage=document.createElement('div');stage.className=`image-crop-stage image-crop-${kind}`;const cropCanvas=document.createElement('canvas');cropCanvas.setAttribute('role','img');cropCanvas.setAttribute('aria-label',`${title}の編集中プレビュ`);stage.append(cropCanvas);
  ...
  const applyView=()=>drawCrop(cropCanvas,editor,kind);
  ...
  function clearSource(){if(sourceUrl)URL.revokeObjectURL(sourceUrl);sourceUrl=null;editor.source.removeAttribute('src');cropCanvas.getContext('2d').clearRect(0,0,cropCanvas.width,cropCanvas.height);}
